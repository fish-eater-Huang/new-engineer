...
void IMU::update(void) {
  ...
  memcpy(gyro_world_, &mahony_.ww_[0][0], 3 * sizeof(float));
  memcpy(accel_world_, &mahony_.aw_[0][0], 3 * sizeof(float));
  memcpy(mag_world_, &mahony_.mw_[0][0], 3 * sizeof(float));
  for (int i = 0; i < 3; i++) {
