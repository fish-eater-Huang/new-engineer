...
struct DrawProperty {
    int priority = 0;
    uint16_t count_since_last_update = 0; // 一次 count 的实际时间为 refresh_delay_ ms
    uint8_t min_request_refresh_count = 100; // 默认100次handle一定要刷新一次
    ////TODO 刷新
    bool static_content = false; // 如果为真，则不会主动刷新，除非操作手按指定按键触发刷新ui
    bool drawn = false;
 ...
 *        此类对象一旦被创建且作为参数传入 manager 类，就会被认为是一个画图请求。删除此图形也属于画图请求，但是不需要创建对象，而通过
...
public:
    ...
    Graph *setPos(u16 x,u16 y,bool if_start_point = true) { // if start point默认修改start point, 设置为false修改end point
        if (if_start_point){
    ...
    Graph *setPos(UIPoint point,bool if_start_point = true){return setPos(point.x,point.y,if_start_point);}
    Graph *setPos(UIVec vec, bool if_start_point = true){return setPos(vec.x, vec.y, if_start_point);}
    Graph *setCompletePos(UIVecWithStartPoint vec){setPos(vec.start_point.x, vec.start_point.y, true);return setPos(
                polar2Point(vec.vec) + vec.start_point, false);}
    Graph *setColor(Color color) { graph_data_.color = color;return this;}
    Graph *setRelativePos(const char target[3],u16 relative_x,u16 relative_y,bool if_start_point = true  );
    Graph *setRelativePos(const char target[3], uint16_t (* func1 )(uint16_t) , uint16_t (* func2 )(uint16_t), bool if_start_point = true);
    Graph *setRelativePos(const char target[3], UIPoint point , bool if_start_point = true){return setRelativePos(target, point.x, point.y, if_start_point);};
    Graph *setRelativePos(const char target[3], UIVec vec , bool if_start_point = true){return setRelativePos(target, vec.x, vec.y, if_start_point);};
    Graph *setFollowColor(const char target[3]);
    Graph *setLine(int16_t x, int16_t y){return setPos(this->graph_data_.start_x + x, this->graph_data_.start_y + y, false);};// 根据直线的第一个点坐标确定另一个点坐标
    Graph *setLine(UIPoint vec){return setPos(vec + UIPoint{(int16_t)this->graph_data_.start_x, (int16_t)this->graph_data_.start_y},
                                              false);};
    Graph *setLine(UIPolarVec vec){return setPos(
                polar2Point(vec) + UIPoint{(int16_t)this->graph_data_.start_x, (int16_t)this->graph_data_.start_y},
                false);};
    ...
    void calcThisPriority() { if(priority_calc_func_ != nullptr) this_priority_ = priority_calc_func_(draw_property_); }
    ...
    void setOperateType(GraphOperateType ot) {
        ...
        if (draw_property_.drawn && ot == ADD)draw_property_.content_changed = false;
    }
    ...
    void setDrawn() {
        ...
        if(graph_data_.operate_type == ADD)draw_property_.content_changed = false;
    }
...
protected:
    graphic_data_struct_t graph_data_{}; //graphic_data_struct_t 只包含能够定义图形本身的部分数据，比如类型，大小，位置，颜色，但是不包含优先级，上次更新时间
    DrawProperty draw_property_{};
    int this_priority_ = 0; // 设置的 priority 不是最终的 priority ，实际比较需要此动态值
    int (*priority_calc_func_)(const DrawProperty &) = nullptr;
};

class Circle : public Graph {
public:
    Circle(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 radius);
};

class Rect : public Graph {
public:
    Rect(const char graphic_name[3], u8 layer, Color color, u8 width, u16 x1, u16 y1, u16 x2, u16 y2);
};

class Line : public Graph {
public:
    Line(const char graphic_name[3], u8 layer, Color color, u8 width, u16 x1, u16 y1, u16 x2, u16 y2);
};

class Ellipse : public Graph {
public:
    Ellipse(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 ax, u16 ay);
};

class Arc : public Graph {
public:
    Arc(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 ax, u16 ay,
        u16 angle_st, u16 angle_ed);
};

class IntNum : public Graph {
public:
    IntNum(const char graphic_name[3], u8 layer, Color color, u8 width, u8 size, u16 x, u16 y, int32_t num);
};

class FloatNum : public Graph {
public:
    FloatNum(const char graphic_name[3], u8 layer, Color color, u8 width, u8 size, u8 effective_digit,
             u16 x, u16 y, float num);
};

class Str : public Graph {
public:
    Str(const char graphic_name[3], u8 layer, Color color, u8 size, u8 len, u8 width, u16 x, u16 y,
...
public:
    ...
    void deleteGraph(uint8_t all, uint8_t layer); //all: 0:delete one layer_; 1:delete all layers
private:
    // ui 画图用到的变量，用户不需要去管
    char char_buf[30]{};
    uint8_t buf[120]{};
...
private:
    ...
    GraphListNode* fineNode(const char name[3]){
        ...
        while (node != nullptr){
            ...
            if (tmp[0] == name[0] && tmp[1] == name[1] && tmp[2] == name[2])return node;
            node = node->next;
        }
        return nullptr;
    }
