...
bool compareGraphicData(graphic_data_struct_t gd1, graphic_data_struct_t gd2){
    bool r = true;
    if(gd1.color != gd2.color
       || gd1.start_angle != gd2.start_angle
       || gd1.end_angle != gd2.end_angle
       || gd1.width != gd2.width
       || gd1.start_x != gd2.start_x
       || gd1.start_y != gd2.start_y
       || gd1.end_x != gd2.end_x
       || gd1.end_y != gd2.end_y
       || gd1.radius != gd2.radius
       || gd1.layer != gd2.layer
            )r = false;
    return r;
}
...
Circle::Circle(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 radius) {
...
Rect::Rect(const char graphic_name[3], u8 layer, Color color, u8 width, u16 x1, u16 y1, u16 x2, u16 y2) {
    graph_data_.graphic_name[0] = graphic_name[0];
    graph_data_.graphic_name[1] = graphic_name[1];
    graph_data_.graphic_name[2] = graphic_name[2];
    graph_data_.operate_type = NO_OPERATE;
...
Line::Line(const char graphic_name[3], u8 layer, Color color, u8 width, u16 x1, u16 y1, u16 x2, u16 y2) {
    graph_data_.graphic_name[0] = graphic_name[0];
    graph_data_.graphic_name[1] = graphic_name[1];
    graph_data_.graphic_name[2] = graphic_name[2];
    graph_data_.operate_type = NO_OPERATE;
...
Ellipse::Ellipse(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 ax,
                 u16 ay) {
    graph_data_.graphic_name[0] = graphic_name[0];
    graph_data_.graphic_name[1] = graphic_name[1];
    graph_data_.graphic_name[2] = graphic_name[2];
...
Arc::Arc(const char graphic_name[3], u8 layer, Color color, u8 width, u16 center_x, u16 center_y, u16 ax, u16 ay,
...
IntNum::IntNum(const char graphic_name[3], u8 layer, Color color, u8 width, u8 size, u16 x, u16 y, int32_t num) {
    graph_data_.graphic_name[0] = graphic_name[0];
    graph_data_.graphic_name[1] = graphic_name[1];
    graph_data_.graphic_name[2] = graphic_name[2];
    graph_data_.operate_type = NO_OPERATE;
...
FloatNum::FloatNum(const char graphic_name[3], u8 layer, Color color, u8 width, u8 size,
 ...
 *         output is supported, using this function will waste precious 10hz limit
 ******************************************************************************
**/
Str::Str(const char graphic_name[3], u8 layer, Color color, u8 size, u8 len, u8 width, u16 x, u16 y,
...
Graph *Graph::setRelativePos(const char target[3],u16 relative_x,u16 relative_y,bool if_start_point){
    auto data = UI::getGraphData(target);
    return this->setPos(data.start_x + relative_x,data.start_y + relative_y,if_start_point);
}
Graph *Graph::setRelativePos(const char target[3], uint16_t (* func1 )(uint16_t) , uint16_t (* func2 )(uint16_t), bool if_start_point){
    auto data = UI::getGraphData(target);
...
UI &UI::add(Graph* graph) {
    ...
    while (node != nullptr) {
        if (compareName(node->graph->getGraphicName(), graph->getGraphicName())) {
            ...
            // 但是如果只是 Graph 类的属性发生了变化（比如改变了 priority 计算规则）但是不修改图形，则此函数不会起作用
//            if (GetCRC(node->graph->getGraphData()) == GetCRC(graph->getGraphData())) {
            if (compareGraphicData(node->graph->getGraphData(), graph->getGraphData())) {
                // 完全一样的图形
                delete graph;
                return *this;
            }
...
UI &UI::del(const char graphic_name[3]) {
    ...
    while (node != nullptr) {
        ...
        for (int i = 0; i < 3; ++i) {
            if (it_name[i] != graphic_name[i])break;
            if (i == 2) {
 ...
 * @param graphs
 * @param start_index
 * @param num the overall size of the list, we need this param because the actual size of list is always 7, but may only need less elements
 *            note that this param is not the number of element you want to move but the used size of list
 */
inline void moveGraphs(GraphListNode **graphs, uint8_t start_index, uint8_t num) {
...
void UI::graphManagerHandle() {
    ...
    while (node->graph->getGraphData().graphic_type == STRING && node->next != nullptr) node = node->next; // 防止string把队列卡死

    draw_nums = graph_num_<7 ? graph_num_ : 7;  // 节约带宽，同时避免删除操作被多次执行

    for (uint8_t i = 0; i < draw_nums; ++i) {
        first_seven_graphs[i] = node;
        node = getCircularNextNode(node);  // 如果前七个全都是优先级最高的，那排序就会失效，而往往最后加入的优先级很高（最后加入的会变成head）
    }
    ...
    while (node != nullptr) {
        ...
        for (int i = 0; i < draw_nums; ++i) { // 排序排出前n个，取决于用户设定
            if(compareName(node->graph->getGraphicName(), first_seven_graphs[i]->graph->getGraphicName()))break;
            if (node->graph->getThisPriority() > first_seven_graphs[i]->graph->getThisPriority()) {
...
//        if (state.content_changed) { addition +=20; }
    ...
    if (state.min_request_refresh_count - state.count_since_last_update < 30)
        ...
        addition += state.priority + (30 - state.min_request_refresh_count + state.count_since_last_update) * 2;
...
int defaultPriRule(const DrawProperty &state) {
    ...
    if (state.min_request_refresh_count - state.count_since_last_update < 5)
        // 如果全都挤到一起发送反而也会很卡，这里先分配出去一些
        addition += state.priority + (5 - state.min_request_refresh_count + state.count_since_last_update) * 10;
    if (state.min_request_refresh_count <= state.count_since_last_update)
        addition += (state.count_since_last_update - state.min_request_refresh_count + 1) * 100000;
...
void UI::handle() {
    ...
    if (HAL_GetTick() -last_tick_  >= refresh_delay_) { //8hz
        ...
        graphManagerHandle();
        if (++seq_ >= 100) seq_ = 1;
        if (graph_n_ > 0) drawGraphs(graph_n_);
